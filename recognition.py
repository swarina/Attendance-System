...
def recognise():
    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            if confidence >= 80:
                id = -1
            # print(id)
            cur.execute("SELECT name FROM students WHERE roll_no = {s_roll_no}".format(s_roll_no = id))
            temp = cur.fetchall()
    ...
    if id == -1:
        ...
        if not cur.fetchone()[0]:
            ...
            conn.commit()
        print(id, date_today)
        cur.execute("UPDATE attendance SET _{s_roll_no}=1 WHERE date={d}".format(s_roll_no=id,d=date_today))
        conn.commit()
